...
pub fn compile(allocator: std.mem.Allocator, tokens: tokenizer.TokenList, comptime options: CompileOptions) error{OutOfMemory}![]u8 {
    ...
    while (state.current < tokens.tokens.len) : (state.current += 1) {
        try appendToken(&state, options.ignore_paragraph);
    }
...
fn appendToken(
    ...
    try switch (state.tokens[state.current].token_type) {
        ...
        .code => appendRecusive(state, &.{"code"}, true, true),
        ...
        .horizontal_rule => appendText(state, "<hr/>", true),
        .image => appendImage(state, ignore_paragraph),
        .link => appendLink(state, ignore_paragraph),
        .ampersand => appendText(state, "&amp;", ignore_paragraph),
        .html_start => appendText(state, "&lt;", ignore_paragraph),
        .html_end => appendText(state, "&gt;", ignore_paragraph),
        else => {
            try appendHtml(state, null, false);
        },
    };
}
...
fn appendRecusive(
    ...
    if (!ignore_paragraph and state.is_in_paragraph) try endParagraph(state);
    if (tags) |t| inline for (t) |tag| try state.html.writeAll("<" ++ tag ++ ">");

    const inner_html = try compile(
        state.allocator,
        token.value.children,
        .{ .ignore_paragraph = ignore_paragraph_children },
    );
    ...
    if (tags) |t| inline for (t) |tag| try state.html.writeAll("</" ++ tag ++ ">");
    if (!ignore_paragraph and state.tokens[state.current].has_following_space)
        try state.html.writeByte(' ');
}
...
fn appendList(
    ...
    while (list_state.current < list_state.tokens.len) : (list_state.current += 1)
        try appendRecusive(&list_state, &.{"li"}, true, false);
...
fn appendCodeBlock(
    ...
    for (token.value.children.tokens[offset..], 0..) |_, i| {
        ...
        try appendHtml(&code_state, null, true);
    }
...
fn appendImage(
    ...
    if (!ignore_paragraph and !state.is_in_paragraph) try startParagraph(state);
